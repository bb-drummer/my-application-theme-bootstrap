	...
	var $doc = $(doc),
		...
		initCTAXHRModals = function () {
			var $body = $('BODY'),
				...
				$ajaxCTAClose = ".modal-content .btn-cta-xhr-close, .modal-content .alert, .modal-content .close, .cta-xhr-modal-close",
			...
			$body.on('submit', $ajaxForms, {}, function (oEvent) {
				...
				formdata.push( 
					(form.find('.modal-dialog INPUT[name=del].btn').size() > 0) ? {name: 'del', value: 'delete'} : null 
				);
